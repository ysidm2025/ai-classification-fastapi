...
def classify_conversation(conversation_id, merged_messages):
    ...
    messages = [
        ...
        You are an AI assistant evaluating chatbot performance.
            ...
            Feedback: The chatbot incorrectly classified this conversation. as the bot was providing correct reply of providing avilable options of tours but as live video tour was not available it was not bot's fault but the user keeps asking for live video tour when bot already told it is currently unavailable so this conversation is Successful
            Feedback: The chatbot incorrectly classified this conversation the bot did not provide information about property whe asked by user not even any ameneties or any relevant information rather it asked you can tour the property .It also did not answer when asked about application fee but asked to take a tour so this conversation is Unsuccessful
            Feedback: The chatbot incorrectly classified this conversation as this conversation is unsuccessful because when user asked Pricing & Availability bot replied with Sorry, I missed what you just said and did not give needed info but gave second time when asked same quesion by user.
            Feedback: The chatbot incorrectly classified this conversation. When the user asked how 3 bedrooms? the bot totally missed the users need and gave wrong asnwer about pet policy then when user asked same question again bot asked if it can help with anything else which is not a valid response to how 3 bedrooms? so this conversation is Unsuccessful
            Feedback: The chatbot incorrectly classified this conversation. when the user asked about different nearby places bot replied with a map link whuch is correct response as user can check the map and get nearby places so this conversation is Successful.
            Feedback: The chatbot incorrectly classified this conversation.bot asnwered correctly to schedule a tour provided options for guided self or live video and asked for users information to schedule tour but it was user's mistake that he didnt provide with correct number because of which the conversation was repeated twice and bot asked user for phone number so this conversation is Successful.
            Feedback: The chatbot incorrectly classified this conversation.The conversation is Unsuccessful because when user asked about property number and property phone it should have provided with call us at number which it did later in conversaion .But here it replied with i am a virtual assistand and did not understand.Feedback: The chatbot incorrectly classified this conversation. as user asked Do you guys have a pet limit? and bot replied thank you for your information which is not a valid response here shoulf have given information about pet limit.
            Feedback: The chatbot incorrectly classified this conversation. The bot asked user to wait to check user manual when user asked about friend park in parking spot where it should have answered yes or no or relevant answer and asked to connect to leasing agent when user asked about gym available where bot should have told yes or no or any relevant answer so these make conversation Unsuccessful
            Feedback: The chatbot incorrectly classified this conversation.The user asked about pet policy 3 times and bot did not tell user any when it should've told very first time so it is Unsuccessful fourth time it told user when he asked again but as he did not answer first 3 times or ver first time correctly it is Unsuccessful
        """},
...
def convert_file():
    ...
    # file_path = "test_set_2.csv"  # Replace with your CSV file path
    ...
    # output_path = "test_set_2.csv"  # Specify the desired output file name
...
# file_path = "test_set_2.csv"  # Path to your test_set.csv file
# df = pd.read_csv(file_path)
...
# output_path = "test_set_updated_2.csv"  # Path to your test_set.csv file
